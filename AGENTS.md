...
이 저장소는 공개된다. 한 번 push된 시크릿은 **삭제해도 유출된 것으로 취급**해야 한다.

- 비밀값은 `.env.local` 에 두고 코드에서는 `process.env` 로 읽는다
- **`NEXT_PUBLIC_` 접두사를 비밀값에 붙이지 않는다.** 이 접두사는 브라우저 번들에 값을 그대로 노출한다
- 배포 환경변수는 저장소가 아니라 호스팅 대시보드(Vercel 등)에 둔다
- 커밋 전 `.githooks/pre-commit` 이 스테이징된 내용을 검사한다. 클론 후 아래를 1회 실행해야 동작한다

  ```bash
  git config core.hooksPath .githooks
  ```

- 훅이 막았는데 실제 시크릿이라면 **커밋을 고치기 전에 발급처에서 먼저 폐기(rotate)** 한다
